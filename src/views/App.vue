<template>
  <NcContent app-name="notebook">
    ...
    <NcAppContent>
      ...
      />
      <NcEmptyContent v-else :title="t('tutorial_5', 'Select a note')">
        <template #icon>
          <NoteIcon :size="20" />
        </template>
      </NcEmptyContent>
...
export default {
  ...
  methods: {
    onEditNote(noteId, content) {
      ...
      const url = generateOcsUrl("apps/notebook/api/v1/notes/{noteId}", {
        noteId,
      });
      axios
        .put(url, options)
        .then((response) => {
          ...
          this.notesById[noteId].last_modified =
            response.data.ocs.data.last_modified;
        })
        .catch((error) => {
    ...
    deleteNote(noteId) {
      ...
      axios
        .delete(url)
        .then((response) => {
          const indexToDelete = this.state.notes.findIndex(
            (n) => n.id === noteId
          );
          if (indexToDelete !== -1) {
    ...
    onExportNote(noteId) {
      const url = generateOcsUrl("apps/notebook/api/v1/notes/{noteId}/export", {
        noteId,
      });
      axios
        .get(url)
        .then((response) => {
          showSuccess(
            t("notebook", "Note exported in {path}", {
              path: response.data.ocs.data,
            })
          );
        })
        .catch((error) => {
